...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
const LANGS = {
  de:{label:'DEU'},en:{label:'ENG'},ro:{label:'ROM'},
  pl:{label:'POL'},cs:{label:'CZE'},bg:{label:'BUL'},ru:{label:'RUS'},
};
...
/* ── Icons (statt Emojis) ── */
function IconTruck({size=20,color="#fff"}){return(<svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M10 17h4V5H2v12h3"/><path d="M14 8h4l4 4v5h-3"/><circle cx="7.5" cy="17.5" r="1.6"/><circle cx="17.5" cy="17.5" r="1.6"/></svg>);}
function IconSun({size=18,color="currentColor"}){return(<svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"/></svg>);}
function IconMoon({size=18,color="currentColor"}){return(<svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8z"/></svg>);}
function IconGlobe({size=15,color="currentColor"}){return(<svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M3 12h18"/><path d="M12 3a14 14 0 0 1 0 18a14 14 0 0 1 0-18z"/></svg>);}
        ...
        <nav style={{borderBottom:`1px solid ${C.border}`,background:C.navBg,backdropFilter:'blur(12px)',position:'sticky',top:0,zIndex:100,transition:'background 0.3s'}}>
          <div className="wrap" style={{display:'flex',alignItems:'center',justifyContent:'space-between',height:56,gap:16}}>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <div style={{background:'linear-gradient(135deg,#f97316,#ea580c)',borderRadius:10,width:34,height:34,display:'flex',alignItems:'center',justifyContent:'center'}}><IconTruck size={20}/></div>
            ...
            <div style={{display:'flex',alignItems:'center',gap:24}}>
              ...
              <div style={{position:'relative'}}>
                <button onClick={()=>setLangOpen(!langOpen)} style={{background:C.langBg,border:`1px solid ${C.border}`,borderRadius:8,padding:'6px 12px',fontSize:13,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:6,color:C.txt,transition:'all 0.15s'}}>
                  <IconGlobe/> {LANGS[lang].label} <span style={{fontSize:9,color:C.dim}}>▼</span>
                </button>
                {langOpen&&(
                ...
                <div className="presets">
                  {t.presets.map(p=>(
                    <button key={p.planned} className="preset-btn" onClick={()=>setPlanned(p.planned)} style={{background:planned===p.planned?C.acc:C.presetBg,color:planned===p.planned?'#fff':C.muted,border:`1px solid ${planned===p.planned?C.acc:C.border}`,borderRadius:8,padding:'7px 12px',fontSize:12,cursor:'pointer',fontWeight:planned===p.planned?700:500,display:'flex',alignItems:'center',gap:5}}>
                      <span>{p.name}</span><span style={{opacity:0.6,fontSize:11}}>{p.sub}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <article style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:'24px 24px',marginBottom:16}}>
                ...
                <div style={{background:C.errorBg,border:`1px solid rgba(239,68,68,0.25)`,borderRadius:10,padding:'12px 16px',marginTop:12}}>
                  <p style={{fontSize:13,color:C.error,lineHeight:1.7,margin:0}}>
                    <strong>Wichtig:</strong> Die regul&auml;re w&ouml;chentliche Ruhezeit (45h) darf nicht im Fahrzeug verbracht werden (Art. 8 Abs. 8). Bei Versto&szlig; drohen dem Fahrer bis zu 60 € und dem Unternehmer bis zu 500 € pro Fall; der gesetzliche Bu&szlig;geldrahmen reicht bei schweren Verst&ouml;&szlig;en bis zu 30.000 € (&sect; 8a FPersG).
                  </p>
                </div>
              </article>
